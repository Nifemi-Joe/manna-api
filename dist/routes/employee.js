...
const employeeRoutes = async (fastify) => {
    ...
    fastify.get('/menus', async (req, reply) => {
        ...
        const menu = dbGet(`SELECT id, week_start FROM menus WHERE published = 1
       ORDER BY week_start DESC LIMIT 1`);
        if (!menu) {
            return reply.status(404).send({ message: 'No menu published yet' });
        }
        const days = dbAll(`SELECT date, cutoff_time, meal_id
       ...
       ORDER BY date, meal_id`, [menu.id]);
        ...
        for (const row of days) {
            ...
            const meal = dbGet('SELECT * FROM meals WHERE id = ? AND available = 1', [row.meal_id]);
            if (meal) {
    ...
    fastify.get('/employee/allowance', async (req) => {
        ...
        let ledger = dbGet('SELECT amount, used FROM allowance_ledger WHERE user_id = ? AND date = ?', [user.id, today]);
        if (!ledger) {
            const amount = rules?.daily_amount ?? 2500;
            dbRun(`INSERT OR IGNORE INTO allowance_ledger (id, user_id, date, amount, used, reset_at)
         VALUES (?, ?, ?, ?, 0, ?)`, [nanoid(), user.id, today, amount, resetAt]);
    ...
    fastify.get('/orders/me', async (req) => {
        const user = await req.requireAuth();
        const orders = dbAll(`SELECT * FROM orders WHERE user_id = ? ORDER BY created_at DESC LIMIT 50`, [user.id]);
        return {
    ...
    fastify.post('/orders', async (req, reply) => {
        ...
        const meal = dbGet('SELECT * FROM meals WHERE id = ? AND available = 1', [mealId]);
        if (!meal)
            return reply.status(404).send({ message: 'Meal not found or unavailable' });
        // Check cutoff hasn't passed
        const menuMeal = dbGet(`SELECT mm.cutoff_time FROM menu_meals mm
       JOIN menus m ON m.id = mm.menu_id
       WHERE mm.meal_id = ? AND mm.date = ? AND m.published = 1`, [mealId, date]);
        if (!menuMeal)
        ...
        const existing = dbGet('SELECT id FROM orders WHERE user_id = ? AND date = ? AND status != ?', [user.id, date, 'cancelled']);
        if (existing)
        ...
        if (!ledger) {
            const resetAt = `${today}T23:59:59.999Z`;
            dbRun(`INSERT OR IGNORE INTO allowance_ledger (id, user_id, date, amount, used, reset_at)
         VALUES (?, ?, ?, ?, 0, ?)`, [nanoid(), user.id, today, dailyAllowance, resetAt]);
        ...
        const company = dbGet('SELECT address FROM companies WHERE id = ?', [user.companyId]);
        dbRun(`INSERT INTO orders (id, user_id, company_id, meal_id, meal_name, date, status, total_amount, allowance_covered, employee_paid, delivery_address, notes, cancellable)
       VALUES (?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?, ?, ?, 1)`, [orderId, user.id, user.companyId, mealId, meal.name, date, meal.price, allowanceCovered, employeePaid, company?.address ?? '', notes ?? null]);
        // Create delivery record
        dbRun(`INSERT INTO deliveries (id, order_id, company_id, status, delivery_address, scheduled_for)
       VALUES (?, ?, ?, 'scheduled', ?, ?)`, [nanoid(), orderId, user.companyId, company?.address ?? '', `${date}T12:30:00.000Z`]);
        // Deduct from allowance ledger
        dbRun('UPDATE allowance_ledger SET used = used + ? WHERE user_id = ? AND date = ?', [allowanceCovered, user.id, today]);
    ...
    fastify.patch('/orders/:id/cancel', async (req, reply) => {
        ...
        const { id } = req.params;
        const order = dbGet('SELECT * FROM orders WHERE id = ? AND user_id = ?', [id, user.id]);
        if (!order)
        ...
        dbRun(`UPDATE orders SET status = 'cancelled', cancellable = 0, updated_at = datetime('now') WHERE id = ?`, [id]);
        // Refund allowance
        const today = new Date().toISOString().slice(0, 10);
        dbRun('UPDATE allowance_ledger SET used = MAX(0, used - ?) WHERE user_id = ? AND date = ?', [order.allowance_covered, user.id, today]);
        return { success: true };
    });
    // POST /api/v1/payments/paystack/initialize
    fastify.post('/payments/paystack/initialize', async (req, reply) => {
        ...
        dbRun(`INSERT INTO top_ups (id, user_id, order_id, amount, reference, status)
       VALUES (?, ?, ?, ?, ?, 'pending')`, [nanoid(), user.id, body.data.orderId ?? null, body.data.amount, reference]);
