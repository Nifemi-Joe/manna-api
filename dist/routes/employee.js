...
const MEAL_WINDOWS = ['breakfast', 'lunch'];
...
function inferWindowFromTime() {
    const hour = new Date().getUTCHours();
    return hour < BREAKFAST_WINDOW_END_HOUR ? 'breakfast' : 'lunch';
}
function parseWindow(raw) {
    if (MEAL_WINDOWS.includes(raw))
        return raw;
    return inferWindowFromTime();
}
function defaultCutoffIso(date, window) {
...
function formatMealForResponse(meal) {
    return {
        id: meal.id,
        name: meal.name,
...
async function resolveAllowanceAmount(payerId, companyId, window) {
    const payer = await dbGet('SELECT allowance_override_lunch, allowance_override_breakfast, level_id FROM users WHERE id = $1', [payerId]);
    const override = window === 'breakfast' ? payer?.allowance_override_breakfast : payer?.allowance_override_lunch;
    if (override != null)
        return toMoney(override);
    if (payer?.level_id) {
        const level = await dbGet('SELECT daily_amount_lunch, daily_amount_breakfast FROM staff_levels WHERE id = $1', [payer.level_id]);
        const levelAmount = window === 'breakfast' ? level?.daily_amount_breakfast : level?.daily_amount_lunch;
        if (levelAmount != null)
            return toMoney(levelAmount);
    }
    const rules = await dbGet('SELECT daily_amount, daily_amount_breakfast FROM allowance_rules WHERE company_id = $1', [companyId]);
    if (window === 'breakfast')
        return toMoney(rules?.daily_amount_breakfast, 0);
    return toMoney(rules?.daily_amount, 2500);
}
async function resolveOverspendLimit(payerId, window) {
    const payer = await dbGet('SELECT level_id FROM users WHERE id = $1', [payerId]);
    if (!payer?.level_id)
        return 0;
    const level = await dbGet('SELECT overspend_limit_lunch, overspend_limit_breakfast FROM staff_levels WHERE id = $1', [payer.level_id]);
    const limit = window === 'breakfast' ? level?.overspend_limit_breakfast : level?.overspend_limit_lunch;
    return toMoney(limit, 0);
}
async function resolveCanOrderForOthers(userId) {
    const user = await dbGet('SELECT level_id FROM users WHERE id = $1', [userId]);
    if (!user?.level_id)
        return false;
    const level = await dbGet('SELECT can_order_for_others FROM staff_levels WHERE id = $1', [user.level_id]);
    return level?.can_order_for_others === true;
}
async function checkWorkingDay(companyId, dateStr) {
    const rules = await dbGet('SELECT eligible_days FROM allowance_rules WHERE company_id = $1', [companyId]);
    ...
    const holiday = await dbGet('SELECT label FROM company_holidays WHERE company_id = $1 AND date = $2', [companyId, dateStr]);
    if (holiday) {
        return `Ordering is closed today for ${holiday.label}.`;
    }
    return null;
}
async function getOrCreateLedger(payerId, companyId, date, window) {
    let ledger = await dbGet('SELECT amount, used FROM allowance_ledger WHERE user_id = $1 AND date = $2 AND meal_window = $3', [payerId, date, window]);
    if (!ledger) {
        const amount = await resolveAllowanceAmount(payerId, companyId, window);
        const resetAt = `${date}T23:59:59.999Z`;
        await dbRun(`INSERT INTO allowance_ledger (id, user_id, date, amount, used, reset_at, meal_window)
             VALUES ($1, $2, $3, $4, 0, $5, $6)
                 ON CONFLICT (user_id, date, meal_window) DO NOTHING`, [nanoid(), payerId, date, amount, resetAt, window]);
        ledger = { amount, used: 0 };
    }
    return ledger;
}
async function availableMealsFor(w) {
    const rows = await dbAll('SELECT * FROM meals WHERE available = TRUE AND meal_window = $1 ORDER BY name', [w]);
...
const employeeRoutes = async (fastify) => {
    fastify.get('/menus', async (req) => {
        ...
        const menu = await dbGet(`SELECT id, week_start FROM menus WHERE published = TRUE ORDER BY week_start DESC LIMIT 1`);
        if (menu) {
            const scheduledToday = await dbAll(`SELECT date, cutoff_time, meal_id FROM menu_meals
                 WHERE menu_id = $1 AND meal_window = $2 AND date = $3
                 ORDER BY meal_id`, [menu.id, window, today]);
            if (scheduledToday.length > 0) {
                const meals = [];
                for (const row of scheduledToday) {
                    const meal = await dbGet('SELECT * FROM meals WHERE id = $1 AND available = TRUE AND meal_window = $2', [row.meal_id, window]);
                    if (meal)
                        meals.push(formatMealForResponse(meal));
                }
                if (meals.length > 0) {
                    return {
        ...
        const fallbackMeals = await dbAll('SELECT * FROM meals WHERE available = TRUE AND meal_window = $1 ORDER BY name', [window]);
        return {
            week: today,
            mealWindow: window,
            isFallback: true,
    ...
    fastify.get('/employee/allowance', async (req) => {
        ...
        const window = parseWindow(query.window);
        if (!user.companyId)
            return { dailyAmount: 0, daily: 0, remaining: 0, used: 0, resetAt: '', mealWindow: window, overspendLimit: 0, canOrderForOthers: false };
        const today = new Date().toISOString().slice(0, 10);
    ...
    fastify.get('/employee/colleagues', async (req) => {
        ...
        const rows = await dbAll(`SELECT id, name, email FROM users
             ...
             ORDER BY name`, [user.companyId, user.id]);
        return { colleagues: rows.map((r) => ({ id: r.id, name: r.name, email: r.email })) };
    });
    fastify.get('/orders/me', async (req) => {
        const user = await req.requireAuth();
        const orders = await dbAll(`SELECT o.*, recipient.name as recipient_name, payer.name as payer_name
             FROM orders o
             ...
             ORDER BY o.created_at DESC LIMIT 100`, [user.id]);
    ...
    fastify.post('/orders/cart', async (req, reply) => {
        ...
        if (!body.success)
            return reply.status(400).send({ message: 'Invalid cart', errors: body.error.flatten() });
        ...
        if (onBehalfOfUserId && onBehalfOfUserId !== requester.id) {
            const requesterRecord = await dbGet('SELECT level_id FROM users WHERE id = $1', [requester.id]);
            const level = requesterRecord?.level_id
                ? await dbGet('SELECT can_order_for_others FROM staff_levels WHERE id = $1', [requesterRecord.level_id])
                : null;
            ...
            const recipient = await dbGet('SELECT id FROM users WHERE id = $1 AND company_id = $2 AND status = $3', [onBehalfOfUserId, requester.companyId, 'active']);
            if (!recipient)
                return reply.status(404).send({ message: 'That colleague was not found in your company' });
        ...
        for (const item of items) {
            const meal = await dbGet('SELECT * FROM meals WHERE id = $1', [item.mealId]);
            ...
            const window = meal.meal_window === 'breakfast' ? 'breakfast' : 'lunch';
            ...
            const menuMeal = await dbGet(`SELECT mm.cutoff_time FROM menu_meals mm
                 ...
                 WHERE mm.meal_id = $1 AND mm.date = $2 AND mm.meal_window = $3 AND m.published = TRUE`, [item.mealId, date, window]);
            ...
            const existing = await dbGet('SELECT id FROM orders WHERE user_id = $1 AND date = $2 AND meal_window = $3 AND meal_id = $4 AND status != $5', [recipientId, date, window, item.mealId, 'cancelled']);
            if (existing) {
        ...
        const cartId = nanoid();
        const company = await dbGet('SELECT address FROM companies WHERE id = $1', [requester.companyId]);
        const createdOrders = [];
        let cartTotal = 0;
        ...
        for (const { meal, quantity, window } of loadedItems) {
            ...
            await dbRun(`INSERT INTO orders (id, user_id, ordered_by_user_id, company_id, meal_id, meal_name, date, status, total_amount, allowance_covered, overspend_covered, employee_paid, delivery_address, notes, cancellable, meal_window, quantity, cart_id)
                 VALUES ($1, $2, $3, $4, $5, $6, $7, 'pending', $8, $9, $10, $11, $12, $13, TRUE, $14, $15, $16)`, [orderId, recipientId, requester.id, requester.companyId, meal.id, meal.name, date, lineTotal, lineAllowanceCovered, coveredByOverspend, lineEmployeePaid, company?.address ?? '', notes ?? null, window, quantity, cartId]);
            ...
            await dbRun(`INSERT INTO deliveries (id, order_id, company_id, status, delivery_address, scheduled_for)
                 VALUES ($1, $2, $3, 'scheduled', $4, $5)`, [nanoid(), orderId, requester.companyId, company?.address ?? '', `${date}T${scheduledTime}`]);
            const created = await dbGet('SELECT * FROM orders WHERE id = $1', [orderId]);
        ...
        await dbRun('UPDATE allowance_ledger SET used = used + $1 WHERE user_id = $2 AND date = $3 AND meal_window = $4', [cartAllowanceCovered, requester.id, today, cartWindow]);
    ...
    fastify.patch('/orders/:id/cancel', async (req, reply) => {
        ...
        const { id } = req.params;
        const order = await dbGet('SELECT * FROM orders WHERE id = $1 AND (user_id = $2 OR ordered_by_user_id = $2)', [id, user.id]);
        if (!order)
        ...
        await dbRun('UPDATE allowance_ledger SET used = GREATEST(0, used - $1) WHERE user_id = $2 AND date = $3 AND meal_window = $4', [toMoney(order.allowance_covered), payerId, today, window]);
        return { success: true };
    });
...
function formatOrder(o, viewerUserId) {
    return {
        id: o.id,
        userId: o.user_id,
