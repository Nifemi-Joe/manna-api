...
const accessRoutes: FastifyPluginAsync = async (fastify) => {
    ...
    fastify.get('/permissions', async (req) => {
        ...
        const rows = dbAll<{ id: string; key: string; label: string; grp: string }>(
    ...
    fastify.get('/roles', async (req) => {
        ...
        const roles = dbAll<{ id: string; name: string; description: string | null; created_at: string; updated_at: string }>(
            `SELECT id, name, description, created_at, updated_at FROM roles ORDER BY name`
        );

        const result = roles.map(r => {
            const perms = dbAll<{ key: string }>(
                `SELECT p.key FROM permissions p
         JOIN role_permissions rp ON rp.permission_id = p.id
         WHERE rp.role_id = ?`, [r.id]
            );
            const assigned = dbGet<{ cnt: number }>(
                `SELECT COUNT(*) as cnt FROM role_assignments WHERE role_id = ? AND status = 'active'`,
                [r.id]
            );
            return {
                id: r.id,
    ...
    fastify.post('/roles', async (req, reply) => {
        ...
        dbRun(
            `INSERT INTO roles (id, name, description, scope, created_at, updated_at) VALUES (?, ?, ?, 'company', ?, ?)`,
            [id, name, description ?? null, ts, ts]
        );

        for (const key of permissions) {
            const perm = dbGet<{ id: string }>('SELECT id FROM permissions WHERE key = ?', [key]);
            if (perm) dbRun('INSERT OR IGNORE INTO role_permissions (role_id, permission_id) VALUES (?, ?)', [id, perm.id]);
        }

        const role = dbGet<{ id: string; name: string; created_at: string; updated_at: string }>(
    ...
    fastify.patch('/roles/:id', async (req, reply) => {
        ...
        if (name) dbRun('UPDATE roles SET name = ?, updated_at = ? WHERE id = ?', [name, ts, id]);
        if (description !== undefined) dbRun('UPDATE roles SET description = ?, updated_at = ? WHERE id = ?', [description, ts, id]);
        ...
        const updated = dbGet<{ id: string; name: string; description: string | null; created_at: string; updated_at: string }>(
            'SELECT * FROM roles WHERE id = ?', [id]
        );
        const perms = dbAll<{ key: string }>(`SELECT p.key FROM permissions p JOIN role_permissions rp ON rp.permission_id = p.id WHERE rp.role_id = ?`, [id]);
        const assigned = dbGet<{ cnt: number }>(`SELECT COUNT(*) as cnt FROM role_assignments WHERE role_id = ? AND status = 'active'`, [id]);
    ...
    fastify.get('/assignments', async (req) => {
        ...
        return {
            assignments: rows.map(r => {
                ...
                const role = dbGet<{ name: string }>('SELECT name FROM roles WHERE id = ?', [r.role_id]);
                return {
                    id: r.id,
                    userId: r.user_id,
    ...
    fastify.post('/assignments', async (req, reply) => {
        ...
        const role = dbGet<{ id: string; name: string }>('SELECT id, name FROM roles WHERE id = ?', [roleId]);
        if (!role) return reply.status(404).send({ message: 'Role not found' });
       ...
       VALUES (?, ?, ?, ?, 'active')`,
        ...
        const row = dbGet<{ id: string; created_at: string }>('SELECT id, created_at FROM role_assignments WHERE id = ?', [id]);
    ...
    fastify.patch('/assignments/:id', async (req, reply) => {
        ...
        const updated = dbGet<any>('SELECT * FROM role_assignments WHERE id = ?', [id]);
        const user = getUserById(updated.user_id);
        const role = dbGet<{ name: string }>('SELECT name FROM roles WHERE id = ?', [updated.role_id]);
