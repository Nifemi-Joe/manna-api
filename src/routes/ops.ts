...
const opsRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/deliveries', async (req) => {
    ...
    if (query.status)  { params.push(query.status);  conditions.push(`d.status = $${params.length}`); }
    if (query.company) { params.push(query.company); conditions.push(`d.company_id = $${params.length}`); }
  ...
  fastify.patch('/deliveries/:id', async (req, reply) => {
    ...
    const delivery = await dbGet<any>('SELECT * FROM deliveries WHERE id = $1', [id]);
  ...
  fastify.get('/issues', async (req) => {
    ...
    if (query.severity) { params.push(query.severity); conditions.push(`i.severity = $${params.length}`); }
    if (query.company)  { params.push(query.company);  conditions.push(`i.company_id = $${params.length}`); }
  ...
  fastify.patch('/issues/:id', async (req, reply) => {
    ...
    if (body.data.severity)    { params.push(body.data.severity);    updates.push(`severity = $${params.length}`); }
    if (body.data.description) { params.push(body.data.description); updates.push(`description = $${params.length}`); }
    ...
    const updated = await dbGet<any>('SELECT * FROM issues WHERE id = $1', [id]);
  ...
  fastify.get('/menus/:weekStart', async (req, reply) => {
    ...
    const menu = await dbGet<any>('SELECT * FROM menus WHERE week_start = $1', [weekStart]);
  ...
  fastify.patch('/menus/:weekStart', async (req, reply) => {
    ...
    let menu = await dbGet<any>('SELECT * FROM menus WHERE week_start = $1', [weekStart]);
