...
const adminRoutes: FastifyPluginAsync = async (fastify) => {
    ...
    fastify.post('/companies', async (req, reply) => {
        ...
        dbRun(
            `INSERT INTO companies (id, name, slug, plan, status, address, city) VALUES (?, ?, ?, ?, 'active', ?, ?)`,
            [id, name, slug, plan, address, city]
        );

        // Seed allowance rules
        dbRun(
            `INSERT INTO allowance_rules (id, company_id, daily_amount) VALUES (?, ?, 2500)`,
            [nanoid(), id]
        );

        const company = dbGet<any>('SELECT * FROM companies WHERE id = ?', [id]);
    ...
    fastify.patch('/companies/:id', async (req, reply) => {
        ...
        if (body.data.name)   { updates.push('name = ?');   params.push(body.data.name); }
        if (body.data.plan)   { updates.push('plan = ?');   params.push(body.data.plan); }
        if (body.data.status) { updates.push('status = ?'); params.push(body.data.status); }
        if (body.data.address){ updates.push('address = ?');params.push(body.data.address); }
        ...
        const updated = dbGet<any>('SELECT * FROM companies WHERE id = ?', [id]);
    ...
    fastify.get('/users', async (req) => {
        ...
        if (query.portal) { conditions.push('u.portal = ?'); params.push(query.portal); }
    ...
    fastify.patch('/users/:id', async (req, reply) => {
        ...
        const params: unknown[] = [];
        if (body.data.name)   { updates.push('name = ?');   params.push(body.data.name); }
        if (body.data.portal) { updates.push('portal = ?'); params.push(body.data.portal); }
        if (body.data.status) { updates.push('status = ?'); params.push(body.data.status); }
        ...
        const updated = dbGet<any>('SELECT u.*, c.name as company_name FROM users u LEFT JOIN companies c ON c.id = u.company_id WHERE u.id = ?', [id]);
        return {
            user: {
