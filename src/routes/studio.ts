...
const studioRoutes: FastifyPluginAsync = async (fastify) => {
    ...
    fastify.get("/content", async (req) => {
        ...
        const entries = dbAll<any>("SELECT * FROM content_entries ORDER BY section, key");
    ...
    fastify.get("/content/entry", async (req, reply) => {
        ...
        const entry = dbGet<any>("SELECT * FROM content_entries WHERE key = ?", [key]);
    ...
    fastify.patch("/content/entry", async (req, reply) => {
        ...
        let entry = dbGet<any>("SELECT * FROM content_entries WHERE key = ?", [key]);

        if (!entry) {
            dbRun(
                "INSERT INTO content_entries (key, type, title, status, section, content, edited_by, last_edited_at) VALUES (?, \'markdown\', ?, \'draft\', ?, ?, ?, datetime(\'now\'))",
            ...
            dbRun(
                "UPDATE content_entries SET content = ?, status = ?, edited_by = ?, last_edited_at = datetime(\'now\') WHERE key = ?",
                [body.data.content, newStatus, user.name, key]
            );
        }

        const updated = dbGet<any>("SELECT * FROM content_entries WHERE key = ?", [key]);
    ...
    fastify.post("/content/publish", async (req, reply) => {
        ...
        dbRun(
            "UPDATE content_entries SET status = \'published\', last_published_at = ?, last_edited_at = ? WHERE key = ?",
            [now, now, key]
        );
        dbRun(
            "INSERT INTO content_revisions (id, entry_key, content, published_by, published_at, summary) VALUES (?, ?, ?, ?, ?, \'Published\')",
    ...
    fastify.post("/content/rollback", async (req, reply) => {
        ...
        const revision = dbGet<any>(
            "SELECT * FROM content_revisions WHERE id = ? AND entry_key = ?",
            [body.data.revisionId, key]
        );
        if (!revision) return reply.status(404).send({ message: "Revision not found" });

        dbRun(
            "UPDATE content_entries SET content = ?, status = \'unpublished_changes\', edited_by = ?, last_edited_at = datetime(\'now\') WHERE key = ?",
    ...
    fastify.get("/content/revisions", async (req, reply) => {
        ...
        const revisions = dbAll<any>(
            "SELECT * FROM content_revisions WHERE entry_key = ? ORDER BY published_at DESC",
            [key]
        );
    ...
    fastify.get("/media", async (req) => {
        ...
        const assets = dbAll<any>("SELECT * FROM media_assets ORDER BY uploaded_at DESC");
    ...
    fastify.post("/media", async (req, reply) => {
        ...
        try {
            ...
            dbRun(
                "INSERT INTO media_assets (id, filename, url, mime_type, size_bytes, tags, uploaded_by) VALUES (?, ?, ?, ?, ?, \'[]\', ?)",
                [id, data.filename, url, data.mimetype, buffer.byteLength, user.name]
            );

            const asset = dbGet<any>("SELECT * FROM media_assets WHERE id = ?", [id]);
    ...
    fastify.patch("/media/:id", async (req, reply) => {
        ...
        if (body.data.alt !== undefined)  { updates.push("alt = ?");  params.push(body.data.alt); }
        if (body.data.tags !== undefined) { updates.push("tags = ?"); params.push(JSON.stringify(body.data.tags)); }
        ...
        const updated = dbGet<any>("SELECT * FROM media_assets WHERE id = ?", [id]);
