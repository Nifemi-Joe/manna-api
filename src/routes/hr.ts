...
const hrRoutes: FastifyPluginAsync = async (fastify) => {
    ...
    fastify.get('/orders', async (req) => {
        ...
        if (query.startDate) { conditions.push(`o.date >= ?`); params.push(query.startDate); }
        if (query.endDate)   { conditions.push(`o.date <= ?`); params.push(query.endDate); }
        if (query.status)    { conditions.push(`o.status = ?`); params.push(query.status); }

        const where = conditions.join(' AND ');

        const total = (dbGet<{ cnt: number }>(
            `SELECT COUNT(*) as cnt FROM orders o WHERE ${where}`, params
        ))?.cnt ?? 0;

        const totalAmount = (dbGet<{ sum: number }>(
    ...
    fastify.post('/employees', async (req, reply) => {
       ...
       VALUES (?, ?, ?, 'employee', ?, 'active')`,
        ...
        const empRole = dbGet<{ id: string }>(`SELECT id FROM roles WHERE id = 'role-employee'`);
        if (empRole) {
            dbRun(
                `INSERT INTO role_assignments (id, user_id, role_id, assigned_by, status)
         VALUES (?, ?, 'role-employee', ?, 'active')`,
                [nanoid(), id, user.id]
            );
        }
    ...
    fastify.patch('/employees/:id', async (req, reply) => {
        ...
        const emp = dbGet<any>('SELECT * FROM users WHERE id = ? AND company_id = ?', [id, user.companyId]);
        ...
        if (body.data.name) dbRun(`UPDATE users SET name = ?, updated_at = datetime('now') WHERE id = ?`, [body.data.name, id]);
        if (body.data.status) dbRun(`UPDATE users SET status = ?, updated_at = datetime('now') WHERE id = ?`, [body.data.status, id]);

        const updated = dbGet<any>('SELECT * FROM users WHERE id = ?', [id]);
    ...
    fastify.get('/rules', async (req) => {
        ...
        if (!user.companyId) return reply404();

        const rules = dbGet<any>('SELECT * FROM allowance_rules WHERE company_id = ?', [user.companyId]);
        if (!rules) return { dailyAmount: 2500, monthlyCapEnabled: false, mealType: 'lunch', allowTopUps: true, maxTopUp: 5000, maxMealsPerDay: 1, allowAddOns: false, eligibleDays: ['Monday','Tuesday','Wednesday','Thursday','Friday'] };
    ...
    fastify.patch('/rules', async (req, reply) => {
        ...
        if (!existing) {
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            ...
            if (d.dailyAmount !== undefined)      { updates.push('daily_amount = ?');         params.push(d.dailyAmount); }
            if (d.monthlyCapEnabled !== undefined){ updates.push('monthly_cap_enabled = ?');  params.push(d.monthlyCapEnabled ? 1 : 0); }
            if (d.monthlyCap !== undefined)       { updates.push('monthly_cap = ?');          params.push(d.monthlyCap); }
            if (d.mealType !== undefined)         { updates.push('meal_type = ?');            params.push(d.mealType); }
            if (d.allowTopUps !== undefined)      { updates.push('allow_top_ups = ?');        params.push(d.allowTopUps ? 1 : 0); }
            if (d.maxTopUp !== undefined)         { updates.push('max_top_up = ?');           params.push(d.maxTopUp); }
            if (d.maxMealsPerDay !== undefined)   { updates.push('max_meals_per_day = ?');    params.push(d.maxMealsPerDay); }
            if (d.allowAddOns !== undefined)      { updates.push('allow_add_ons = ?');        params.push(d.allowAddOns ? 1 : 0); }
            if (d.eligibleDays !== undefined)     { updates.push('eligible_days = ?');        params.push(JSON.stringify(d.eligibleDays)); }
            if (updates.length) {
        ...
        const updated = dbGet<any>('SELECT * FROM allowance_rules WHERE company_id = ?', [user.companyId]);
    ...
    fastify.get('/billing', async (req) => {
        ...
        const currentDue = (dbGet<{ sum: number }>(
            `SELECT COALESCE(SUM(allowance_covered), 0) as sum FROM orders WHERE company_id = ? AND date LIKE ? AND status NOT IN ('cancelled','failed')`,
            [user.companyId, `${currentMonth}%`]
        ))?.sum ?? 0;
    ...
    fastify.get('/reports', async (req) => {
        ...
        for (let w = 7; w >= 0; w--) {
            ...
            const row = dbGet<{ orders: number; employees: number }>(
                `SELECT COUNT(*) as orders, COUNT(DISTINCT user_id) as employees
         FROM orders WHERE company_id = ? AND date LIKE ? AND status != 'cancelled'`,
                [user.companyId, `${wk}%`]
            );
        ...
        // Top meals
        const topMeals = dbAll<{ meal_name: string; cnt: number }>(
       ...
       GROUP BY meal_name ORDER BY cnt DESC LIMIT 5`,
        ...
        const issues = dbAll<any>(
            `SELECT * FROM issues WHERE company_id = ? ORDER BY created_at DESC LIMIT 10`,
            [user.companyId]
        );
