...
/**
 * Helper: money-like columns (daily_amount, price, total_amount, etc.) may
 * be declared NUMERIC/DECIMAL on the live database rather than INTEGER —
 * node-postgres returns NUMERIC values as strings (e.g. "2500.00") to avoid
 * floating-point precision loss, not as JS numbers. Always coerce through
 * this before doing arithmetic or writing the value back into another
 * column, or a string like "2500.00" can fail with
 * "invalid input syntax for type integer" when re-inserted.
 */
...
const employeeRoutes: FastifyPluginAsync = async (fastify) => {
    ...
    fastify.get('/menus', async (req, reply) => {
        ...
        const menu = await dbGet<{ id: string; week_start: string }>(
            `SELECT id, week_start FROM menus WHERE published = TRUE
        ...
        const days = await dbAll<{ date: string; cutoff_time: string; meal_id: string }>(
        ...
        for (const row of days) {
            ...
            const meal = await dbGet<any>('SELECT * FROM meals WHERE id = $1 AND available = TRUE', [row.meal_id]);
            if (meal) {
    ...
    fastify.get('/employee/allowance', async (req) => {
        ...
        const rules = await dbGet<any>(
            'SELECT * FROM allowance_rules WHERE company_id = $1',
            [user.companyId]
        );
        ...
        let ledger = await dbGet<{ amount: number; used: number }>(
            'SELECT amount, used FROM allowance_ledger WHERE user_id = $1 AND date = $2',
            [user.id, today]
        );

        if (!ledger) {
            const amount = toMoney(rules?.daily_amount, 2500);
            await dbRun(
                `INSERT INTO allowance_ledger (id, user_id, date, amount, used, reset_at)
                 VALUES ($1, $2, $3, $4, 0, $5)
        ...
        const daily = toMoney(rules?.daily_amount, 2500);
    ...
    fastify.post('/orders', async (req, reply) => {
        ...
        const meal = await dbGet<any>('SELECT * FROM meals WHERE id = $1 AND available = TRUE', [mealId]);
        ...
        const menuMeal = await dbGet<any>(
            `SELECT mm.cutoff_time FROM menu_meals mm
             ...
             WHERE mm.meal_id = $1 AND mm.date = $2 AND m.published = TRUE`,
            [mealId, date]
        );
        ...
        const existing = await dbGet(
            'SELECT id FROM orders WHERE user_id = $1 AND date = $2 AND status != $3',
        ...
        const dailyAllowance = toMoney(rules?.daily_amount, 0);
        const today = new Date().toISOString().slice(0, 10);
        let ledger = await dbGet<{ amount: number; used: number }>(
            'SELECT amount, used FROM allowance_ledger WHERE user_id = $1 AND date = $2',
            [user.id, today]
        ...
        const mealPrice = toMoney(meal.price);
        const remaining = Math.max(0, toMoney(ledger.amount) - toMoney(ledger.used));
        const allowanceCovered = Math.min(mealPrice, remaining);
        const employeePaid = mealPrice - allowanceCovered;

        const orderId = nanoid();
        const company = await dbGet<any>('SELECT address FROM companies WHERE id = $1', [user.companyId]);
        ...
        try {
            await dbRun(
                `INSERT INTO orders (id, user_id, company_id, meal_id, meal_name, date, status, total_amount, allowance_covered, employee_paid, delivery_address, notes, cancellable)
                 VALUES ($1, $2, $3, $4, $5, $6, 'pending', $7, $8, $9, $10, $11, TRUE)`,
                [orderId, user.id, user.companyId, mealId, meal.name, date, mealPrice, allowanceCovered, employeePaid, company?.address ?? '', notes ?? null]
            );
        } catch (err: any) {
        ...
        await dbRun(
            'UPDATE allowance_ledger SET used = used + $1 WHERE user_id = $2 AND date = $3',
            [allowanceCovered, user.id, today]
        );

        const order = await dbGet<any>('SELECT * FROM orders WHERE id = $1', [orderId]);
    ...
    fastify.patch('/orders/:id/cancel', async (req, reply) => {
        ...
        const order = await dbGet<any>('SELECT * FROM orders WHERE id = $1 AND user_id = $2', [id, user.id]);
        ...
        await dbRun(
            `UPDATE orders SET status = 'cancelled', cancellable = FALSE, updated_at = now() WHERE id = $1`,
            [id]
        );
        ...
        const today = new Date().toISOString().slice(0, 10);
        await dbRun(
            'UPDATE allowance_ledger SET used = GREATEST(0, used - $1) WHERE user_id = $2 AND date = $3',
            [toMoney(order.allowance_covered), user.id, today]
        );
