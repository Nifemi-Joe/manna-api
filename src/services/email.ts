 ...
 */
function emailShell(bodyHtml: string, opts?: { preheader?: string }): string {
...
function button(label: string, href: string, variant: 'amber' | 'green' | 'coral' = 'amber'): string {
...
function badge(label: string, tone: 'green' | 'amber' | 'coral' = 'green'): string {
    const colors = {
...
function formatNairaForEmail(n: number): string {
...
function logToConsole(opts: { to: string | string[]; subject: string; text: string }) {
...
async function trySend(opts: { to: string | string[]; subject: string; html: string; text: string }): Promise<SendResult> {
    if (isDev) logToConsole(opts);
    ...
    if (isDev) console.log(`📧 Also sent for real via Resend (id: ${data?.id})`);
    return { sent: true };
}
...
export interface MagicLinkResult extends SendResult {
    link: string;
}
...
// ═══════════════════════════════════════════════════════════════════
// 3. Pilot request notification (internal — to sales)
// ═══════════════════════════════════════════════════════════════════

export interface PilotLeadInfo {
    id: string;
    companyName: string;
    contactName: string;
    email: string;
...
export async function sendPilotRequestNotification(lead: PilotLeadInfo): Promise<SendResult> {
...
export async function sendOnboardingEmail(info: OnboardingInfo): Promise<SendResult> {
...
export interface DailyMenuInfo {
    employeeName: string;
...
export async function sendDailyMenuEmail(to: string, info: DailyMenuInfo): Promise<SendResult> {
...
export async function sendOrderCancelledEmail(to: string, recipientName: string, info: OrderEmailInfo): Promise<SendResult> {
    const html = emailShell(`
...
export async function sendOrderDeliveredEmail(to: string, recipientName: string, info: OrderEmailInfo): Promise<SendResult> {
    const html = emailShell(`
...
// ═══════════════════════════════════════════════════════════════════
// 8. Meal unavailable — swap needed
// ═══════════════════════════════════════════════════════════════════

export interface SwapNeededEmailInfo {
    mealName: string;
    date: string;
...
export async function sendSwapNeededEmail(to: string, recipientName: string, info: SwapNeededEmailInfo): Promise<SendResult> {
...
export async function sendEmail(opts: {
    ...
    if (isDev) logToConsole({ to: opts.to, subject: opts.subject, text: opts.text ?? '(no plain-text body provided)' });
