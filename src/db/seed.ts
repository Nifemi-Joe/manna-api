...
async function seed() {
  ...
  for (const p of PERMS) {
    ...
    if (!exists) {
      await dbRun(
        'INSERT INTO permissions (id, key, label, grp) VALUES ($1, $2, $3, $4)',
  ...
  let co2Id = (existingCo2 as any)?.id;
  if (!co2Id) {
    co2Id = nanoid();
    await dbRun(
      `INSERT INTO companies (id, name, slug, plan, status, address, city, employees_count)
  ...
  // ── Allowance rules ──────────────────────────────────────
  const allowancePairs: Array<[string, number]> = [[coId, 3000], [co2Id, 2500]];
    ...
    if (!exists) {
      await dbRun(
        `INSERT INTO allowance_rules (id, company_id, daily_amount, monthly_cap_enabled, meal_type, allow_top_ups, max_top_up, max_meals_per_day)
         VALUES ($1, $2, $3, FALSE, 'lunch', TRUE, 5000, 1)`,
        [nanoid(), cId, dailyAmt]
      );
    }
  }
  ...
  for (const r of SYSTEM_ROLES) {
    ...
    if (!exists) {
      await dbRun(
        `INSERT INTO roles (id, name, scope, company_id) VALUES ($1, $2, 'system', NULL)`,
        [r.id, r.name]
      );
      for (const pk of r.perms) {
        const perm = await dbGet<{ id: string }>('SELECT id FROM permissions WHERE key = $1', [pk]);
        if (perm) {
          await dbRun(
  ...
  for (let i = 0; i < orderedMeals.length; i++) {
    ...
    const existing = await dbGet(
      'SELECT id FROM orders WHERE user_id = $1 AND date = $2',
      [m.userId, date]
    );
    if (!existing) {
